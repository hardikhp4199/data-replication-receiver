...
// check document exist or not in bucket
func ExistsDocument(bucket string, docKey string) (outObjectFlag bool, errOut error) {
	if bucket != "" && docKey != "" {
		collection, err_collection := getCollection(bucket)
		if err_collection != nil {
			errOut = err_collection
		} else {
			existObject, err := collection.Exists(docKey, &gocb.ExistsOptions{})
			if err != nil {
				errOut = err
				log.Println("CB:Error in exist method: " + err.Error())
			} else {
				outObjectFlag = existObject.Exists()
			}
		}
	} else {
		errOut = errors.New("CB:Bucket name and doc key can not be blank or empty")
	}
	return
}
